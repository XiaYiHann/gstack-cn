...
version: 1.1.0
description: |
  Fast headless browser and workflow router for gstack-cn. Use it to QA a site, verify a
  deployment, take annotated screenshots, test responsive layouts, fill forms, handle dialogs,
  and diff page state before and after actions. Also route users to the right follow-up skill
  when they are brainstorming, reviewing a plan, debugging, testing, shipping, auditing design,
  working carefully in production, or upgrading gstack.
allowed-tools:
  - Bash
  - Read
  - AskUserQuestion
